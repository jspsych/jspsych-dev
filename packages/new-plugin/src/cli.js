...
function showHelp() {
  console.log(`
Usage: new-plugin [options]

Creates a new jsPsych plugin package.

Options:
  --name <name>               Name of the plugin package (required)
  --description <desc>        Brief description of the plugin package (required)
...
async function runWithArgs(cwdInfo, args) {
  ...
  const packagePath = `${cwdInfo.destDir}/plugin-${getHyphenateName(args.name)}`;
  if (fs.existsSync(packagePath)) {
