...
function showHelp() {
  console.log(`
Usage: new-extension [options]

Creates a new jsPsych extension package.

Options:
  --name <name>               Name of the extension package (required)
  --description <desc>        Brief description of the extension package (required)
...
async function runWithArgs(cwdInfo, args) {
  ...
  const packagePath = `${cwdInfo.destDir}/extension-${getHyphenateName(args.name)}`;
  if (fs.existsSync(packagePath)) {
